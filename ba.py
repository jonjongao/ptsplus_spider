...
def parse_home_page(url, html):
    ...
    def parse_carousel(pass_id):
        ...
        for c in carousel_list:
            ...
            if db_id is None:
                href = '/season/' + str(pass_id)
            else:
                href = '/season/' + str(db_id)
            item = {'id': str(pass_id), 'db_id': str(db_id), 'name': name, 'caption': '', 'src': base_url + ext,
                    'href': href}
            # print(item)
            pass_id += 1
    ...
    def parse_new_recommend(pass_id):
        ...
        for r in new_recommend:
            ...
            if db_id is None:
                ...
                href = '/season/' + str(db_id)
            item = {'id': str(pass_id), 'db_id': str(db_id), 'name': name, 'description': '', 'episode': episode,
            ...
            if "item_lists" in s['class']:
                for div in s.findAll('div'):
                    ...
                    if db_id is None:
                        ...
                        href = '/season/' + str(db_id)
                    item = {'id': str(pass_id), 'db_id': str(db_id), 'name': name, 'description': '',
                            'episode': episode, 'src': base_url + ext, 'href': href}
                    pass_id += 1
                    items.append(item)
